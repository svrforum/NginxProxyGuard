...
DO $$ BEGIN
    CREATE TYPE public.block_reason AS ENUM (
        'none', 'waf', 'bot_filter', 'rate_limit', 'geo_block', 'exploit_block', 'banned_ip', 'uri_block', 'cloud_provider_challenge', 'cloud_provider_block', 'access_denied', 'filter_subscription'
    );
...
CREATE TABLE IF NOT EXISTS public.global_settings (
    ...
    client_header_buffer_size character varying(20) DEFAULT '4k'::character varying NOT NULL,
    client_max_body_size character varying(20) DEFAULT '100m'::character varying NOT NULL,
    large_client_header_buffers character varying(20) DEFAULT '4 16k'::character varying NOT NULL,
...
-- Global fail2ban jail (#275): counts failures on requests that matched NO
-- configured host — the catch-all traffic a per-host jail can never see, which
-- is why adding 444 to a host's fail codes does nothing. Bans it creates carry
-- proxy_host_id NULL and therefore apply to every host, so it ships DISABLED
-- with action='log'.
CREATE TABLE IF NOT EXISTS public.global_fail2ban (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
...
^/webapi/'::text,
    ...
    trusted_proxy_cidrs text DEFAULT ''::text NOT NULL,
    trusted_proxy_preset text DEFAULT 'none'::text NOT NULL,
    real_ip_header text DEFAULT 'X-Forwarded-For'::text NOT NULL
);
...
-- ship disabled because simple keyword matching produces false positives on legitimate
-- search/CMS query strings. Marking them auto_disabled_at=now() on fresh install means
-- the one-shot upgrade UPDATE (see migration.go) will not re-clobber an admin's
-- conscious choice to re-enable from the UI. (GitHub Issue #123)
...
('c1e7f001-0000-4000-8000-000000000001', 'rfi', 'Dotenv File Access', E'/\\.env(\\.|$|/)', 'request_uri', 'Blocks access to .env config files (.env, .env.local, .env.production, ...)', 'critical', true, true, 23, NULL),
...
-- 01056 to resolve issue 16
ALTER TYPE public.block_reason ADD VALUE IF NOT EXISTS 'access_denied';
-- v2.7.1: filter subscription block reason (Issue #86)
...
-- logs upstream tracking (Issue #109 - show which load-balanced backend served the request)
-- Stored as text to preserve Nginx's comma-separated list on upstream retries (e.g. "10.0.0.1:8080, 10.0.0.2:8080")
...
ALTER TABLE public.logs_partitioned ADD COLUMN IF NOT EXISTS upstream_addr text;
ALTER TABLE public.logs_partitioned ADD COLUMN IF NOT EXISTS upstream_status text;
COMMENT ON COLUMN public.logs_partitioned.upstream_addr IS 'Upstream server address(es) that handled the request. Comma-separated list on retries; last value is the final responder.';
COMMENT ON COLUMN public.logs_partitioned.upstream_status IS 'Upstream status code(s). Comma-separated list on retries; matches upstream_addr order.';
...
-- v2.51.0: operator-configurable trusted proxies so real client IPs survive a
-- CDN/reverse proxy in front of NPG (#278). Defaults reproduce the previous
-- hardcoded behaviour exactly. Canonical execution lives in migration.go upgrades.
...
-- v2.53.0: global fail2ban jail for traffic that matched no configured host
-- (#275). Canonical execution lives in migration.go upgrades.
CREATE TABLE IF NOT EXISTS public.global_fail2ban (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    enabled boolean DEFAULT false NOT NULL,
    max_retries integer DEFAULT 5 NOT NULL,
...
-- v2.13.2 (issue #123): Exploit rule auto-disable marker
