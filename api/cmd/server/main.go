...
func main() {
	...
	proxyHostService := service.NewProxyHostService(
		...
		exploitBlockRuleRepo, certificateRepo, systemLogRepo, nginxManager,
	)
	...
	logHandler := handler.NewLogHandler(logRepo, redisCache, rateLimitRepo)
	...
	securityService := service.NewSecurityService(rateLimitRepo, botFilterRepo, securityHeadersRepo, upstreamRepo, proxyHostRepo, proxyHostService, ipBanHistoryRepo, uriBlockRepo, redisCache, nginxReloader)
	securityHandler := handler.NewSecurityHandler(securityService, auditService)
	settingsService := service.NewSettingsService(globalSettingsRepo, dashboardRepo, backupRepo, proxyHostRepo, redirectHostRepo, certificateRepo, wafRepo, nginxManager, proxyHostService, dockerStatsService, redisCache, cfg.BackupPath)
