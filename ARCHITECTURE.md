...
- **PipelineCanary** (`service/pipeline_canary.go`): 백그라운드 고루틴. 주기적으로 내부 nginx location `GET /__npg_canary?n=<nonce>` 을 쏘고, **그 행이 deadline 내 `logs_partitioned` 에 도착**하는지 확인 → 통과 = 사슬 전체 정상. 미도착 시 access 파일을 중간 체크포인트로 고장 단계 국소화. Phase 1은 **감지 전용**, Phase 2에서 *치유 가능한* 단계에 한해 **auto-heal** 추가 (아래).
...
- **Auto-heal (Phase 2)** (`canHeal`/`attemptHeal` in `service/pipeline_canary.go`, 시임 연결은 `bootstrap/container.go` `wirePipelineCanaryHealer`; 설계 §4.B): 카나리아가 *치유 가능한* 단계를 감지하면 즉시 복구를 시도하고, **다음 프로브가 복구를 검증**(인라인 재프로브 없음). Stage→action:
  - `nginx_write` → `SystemSettingsHandler.ForceEnableRawLog` — DB 플래그 ON 보장 + **항상** `00-raw-logging.conf` 재생성 + `access_raw.log` 재생성 + nginx reload. 부트 전용 `EnsureRawLogEnabled`는 플래그가 이미 true면 no-op이라 "conf 누락 but 플래그 true" 케이스를 못 고치는데, `ForceEnableRawLog`는 무조건 재적용하므로 그 케이스까지 치유.
  - `path_mismatch` / `tail_stalled` → `LogCollector.RestartTail` — `NGINX_ACCESS_LOG` 재해석 후 tail 재오픈(non-blocking, coalescing).
  - `db_insert` / `nginx_unreachable` → **치유 불가 → escalate** (액션 없음, loud 유지). 전자는 DB/insert 경로, 후자는 도달성 문제라 합성 재적용으로 못 고침.
  - **Bounds:** 30분 윈도우당 최대 3회(`healMaxAttempts`/`healWindow`), in-flight 가드(`healMu.TryLock` — 동시 heal 없음). 소진 시 healing 중단하고 `broken`/escalated 유지; 이후 healthy 프로브 1회로 예산 리셋(`healAttempts`/`healWindowStart` 초기화).
  - **Fail-safe 관련 주의:** `RestartTail` 경로는 nginx reload 자체가 없음. `ForceEnableRawLog` 경로는 nginx reload는 거치지만 정적·하드코딩된 `00-raw-logging.conf`만 다시 쓰므로 `nginx -t` 검증을 거치지 않는 `ReloadNginx`(=`reloadNginxInternal`, 테스트 생략)를 사용 — 즉 이 경로는 다른 config 생성 경로(`testAndReloadNginx`)와 달리 reload 전 `nginx -t`를 하지 않는다.
...
- **`/api/v1/health/detailed`** `log_collector` 블록 신규 필드: `pipeline_status` (healthy|broken|unknown) · `canary_failure_stage` · `last_canary_at` · `access_last_flush_seconds_ago` · `modsec_last_flush_seconds_ago` · `error_last_flush_seconds_ago` · `nginx_status_reachable` · `auto_heal_attempts` (현재 윈도우 시도 횟수) · `auto_heal_exhausted` (예산 소진 = `attempts >= 3`, healthy 프로브 시 0으로 리셋) — Phase 2 auto-heal 상태 표면화(`PipelineCanary.HealState`).
